...
export type Region = 'us' | 'india' | 'china' | 'europe' | 'australia' | 'global';
...
export interface RegionConfig {
  label: string;
  flag: string;
  sources: string[];          // NewsAPI source IDs
  domains: string[];          // fallback domain filter
  searchKeywords: string[];   // geographic keywords for query filtering
  stockSymbols: string[];     // Yahoo Finance tickers — top 10 per region
}

export interface CategoryConfig {
  label: string;
  icon: string;
  newsApiQuery: string;       // keyword query sent to NewsAPI
}
...
export const REGION_ORDER: Region[] = ['us', 'india', 'china', 'europe', 'australia', 'global'];
